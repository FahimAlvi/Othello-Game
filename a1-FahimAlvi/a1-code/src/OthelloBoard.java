...
public class OthelloBoard {
		...
		if((validCoordinate(row,col))){
			...
			for(int i=0;i<=7;i++) {
				if (validCoordinate(row+drow, col+dcol)) {
					if (this.board[row + drow][col + dcol] == EMPTY) {
						return EMPTY;
					}
					if (this.board[row + drow][col + dcol] == temp) {
						row = row + drow;
						col = col + dcol;
					}
					else {
						return this.board[row+drow][col+dcol];
					}
				}
			}
		}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		int count=0;
		if(checkForPattern(row,col,drow,dcol)==EMPTY || validCoordinate(row,col)!=true){
           return -1;
		}
		if(getToken(row,col)==player){
			return 0;
		}
		if(checkForPattern(row,col,drow,dcol)==player && getToken(row,col)!=player){
			for(int i=0;i<=7;i++){
                if(getToken(row,col)!=player){
	...
	public boolean move(int row, int col, char player) {
		...
		boolean flag=false;
		if(validCoordinate(row,col)==false || getToken(row,col)!=EMPTY){
			return flag;
		}
		else{
			for (int drow = -1; drow <= 1; drow++) {
				for (int dcol = -1; dcol <= 1; dcol++) {
				    if(checkForPattern(row+drow,col+dcol,drow,dcol)==player){
